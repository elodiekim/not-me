...
- [ ] 커스텀 리워드 상한/검증 (점검에서 발견) — `RewardScreen`이 `Number(customValue) > 0`만 확인해 $999999 같은 값도 통과. 합리적 상한(예: $200)과 소수 자리 제한 추가
...
- [ ] 뒤로가기 버튼 누락: 하단 탭 없는 화면 중 "아직 커밋 전 브라우징" 단계인데 back이 없는 곳
  - Hero의 Nearby Missions (`/hero`) — Home에서 진입, Home으로 돌아갈 방법 없음
  - Hero의 Mission Detail (`/hero/[id]`) — 미션 상세 보고 수락 안 한 채 목록으로 못 돌아감
  - (Searching / Mission Status / Active Mission / Complete / Reward Earned는 이미 커밋된 미션 진행/종료 상태라 의도적으로 back 없음 — 건드리지 말 것)
