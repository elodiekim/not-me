...
- [ ] **Account(계정) 화면에 이메일 표시 추가, 수정은 불가하게** (사용자 요청 · 2026-07-31) — `EditProfileScreen.tsx`(Settings → Account 진입)엔 지금 Name/Phone만 있고 이메일이 아예 안 보임. 이메일은 읽기 전용으로 추가만(수정 불가) — `session.user.email`은 있지만 `useProfile`이 반환하는 `Profile` 타입엔 없을 수 있어 확인 필요. 수정 가능하게 하려면 Supabase Auth `updateUser({ email })` + 재확인 이메일 플로우가 따로 필요해지므로 이번 범위 밖, 읽기 전용 표시만
