...
- [x] **스토리 화면을 단일 스크롤로 재구성** (`app/about/story.tsx` → `AboutStoryScreen`) — 처음엔 `others.PNG` 목업대로 4단계 탭(step 배지+진행점+Continue) 구조로 만들었다가, "이건 개인 이야기지 튜토리얼이 아닌데 탭으로 끊는 게 안 맞는다"는 판단으로 다시 갈아엎음. 지금은 로고+이중언어 타이틀+Proud Cat 다음, 문장을 짧은 fragment 단위로 이어가다가 "바퀴벌레가 나타났다" 대목 뒤에 실제 바퀴벌레 사진, "고양이가 준비돼 있었다" 대목 뒤에 실제 고양이 사진을 본문 흐름 안에 바로 배치. 중간 💭 인용구(이탤릭, 박스 없음)와 마지막 "There should be an app for this." 하이라이터 스타일 인용구(노란 배경, 살짝 회전)까지 이어지고, "That's how NotMe was born"(로고 인라인) + celebrate-cat + "Start Using NotMe" 버튼(`router.replace('/')`)으로 마무리. **Hero Cat 일러스트는 계속 제외** — celebrate-cat이 마스코트 연결을 담당
- [x] 사진은 원본 그대로(크롭 없음) `resizeMode: 'contain'` + 고정 폭(220)으로 크기만 축소, 애니메이션 없음, `Button` 컴포넌트 재사용
