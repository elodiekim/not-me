...
- [x] **스토리 화면을 에디토리얼 스타일로 재구성** (`app/about/story.tsx` → `AboutStoryScreen`) — 3단계 변천사: ① `others.PNG` 목업대로 4단계 탭(step 배지+진행점+Continue) → "개인 이야기인데 튜토리얼처럼 느껴진다"는 피드백으로 폐기 ② 단일 스크롤 + 짧은 fragment 문장 나열 → "왜 다 한 페이지에" 재질문에 카드/타임라인/에디토리얼 3안을 아티팩트로 비교 목업해서 제시 ③ **에디토리얼(3안)로 확정** — 문장을 잘게 쪼개지 않고 자연스러운 좌측정렬 문단으로 이어가다가, "바퀴벌레가 나타났다" 문단 뒤에 실제 바퀴벌레 사진, "고양이가 준비돼 있었다" 문단 뒤에 실제 고양이 사진을 본문 흐름 속에 배치. 💭 인용구(이탤릭, 중앙정렬 단독 줄)로 감정 브레이크를 준 다음, "That day, I had one thought:" + 크고 굵은 pull-quote("There should be an app for this.")로 이어지고, 짧은 노란 밑줄 구분선 → "That's how **NotMe** was born"(로고는 이미지 대신 스타일링된 텍스트: Not=검정, Me=primary, 브러시 로고 느낌만 차용) → celebrate-cat → "Start Using NotMe" 버튼(`router.replace('/')`). **Hero Cat 일러스트는 계속 제외** — celebrate-cat이 마스코트 연결을 담당
