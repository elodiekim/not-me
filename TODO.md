...
- [ ] 히어로 누적 수익(Total Earned) 표시 (사용자 요청 · 2026-07-22) — `ProfileScreen`에 히어로로 완료한 미션들의 `reward_amount` 합계를 보여주는 통계 카드 추가. **실제 정산/출금은 범위 아님**(CLAUDE.md "Payments" 미구현 확정, 사용자 판단) — 순수 읽기 전용 숫자 표시만. `useMissionHistory`가 이미 `role`/`rewardAmount`/`status`를 반환하니 새 훅/쿼리 불필요, 기존 "Requested/Helped" 카운트 카드 옆에 추가하는 정도로 충분
