...
- [x] **중복 요청 가능** (수정 완료 · 2026-07-16) — 활성 미션이 있어도 새 미션을 또 만들 수 있던 문제 해결.
  ...
  - **⚠️ 같은 버그가 `RequestScreen.tsx`에도 그대로 남아있었음 — 재수정 (사용자가 실기기 테스트 중 발견 · 2026-08-30)**: 위에서 `ConfirmLocationScreen`은 `useFocusEffect`로 고쳤는데, **정작 원래 가드가 있던 `RequestScreen`은 그대로 평범한 `useEffect`였음.** 재현: Searching 화면(아무도 수락 안 한 `requested` 상태)을 가만히 지켜보고만 있었는데 **아무 조작 없이 저절로 `/mission-status`로 튕겨나감** — 히어로 수락도 아니고 유저 조작도 아니었음. 원인은 `RequestScreen`이 Reward→Confirm Location→Searching으로 진행하는 동안 스택 맨 아래에 계속 마운트된 채로 살아있고, `useCreateRequest`의 `activeMission` 쿼리 무효화가 화면에 안 보이는 이 인스턴스에도 적용돼서, 포커스 여부와 무관한 `useEffect`가 즉시 `router.replace('/mission-status')`를 쏴버림. `ConfirmLocationScreen`과 동일하게 `useFocusEffect`로 교체
  - 검증: `npx tsc --noEmit` / `npx eslint` 통과. **실기기 확인 완료(2026-08-30)** — Searching 화면이 더 이상 저절로 안 튕김
  - **후속 버그 — Mission Status에서 뒤로가기하면 Reward로 감** (사용자가 실기기 테스트 중 발견 · 2026-08-30): 위 두 가드(`RequestScreen`/`ConfirmLocationScreen`) 다 그냥 `router.replace()`만 써서 **현재 화면 하나만 바꿔치기**하고 그 아래 쌓인 화면은 그대로 둠. 그래서 `RequestScreen` 가드가 먼저 잡으면 뒤로가기 시 Home(정상)인데, 타이밍에 따라 더 안쪽의 `ConfirmLocationScreen` 가드가 잡으면 Home→Request→Reward가 그대로 남아있어서 뒤로가기 시 **Reward로 감**(비정상) — 똑같은 상황인데 어느 가드가 잡느냐에 따라 뒤로가기 결과가 달라지는 비일관성이 근본 원인
    - `useActiveMission.ts`에 `goToActiveMission(router, missionId)` 헬퍼 추가해서 두 가드가 공유: `router.dismissTo('/')`로 Request 플로우 전체를 Home까지 걷어낸 뒤 `router.push('/mission-status')`로 그 위에 얹음 — 어느 가드가 잡든 결과 스택이 항상 `[Home, MissionStatus]`로 동일해짐
    - 검증: `npx tsc --noEmit` / `npx eslint .` 통과, `useRouter()`가 imperative 싱글톤을 그대로 반환하는 것 확인(`dismissTo` 존재 확인). **실기기 재확인 필요** — 두 가드 중 어느 쪽이 잡든 Mission Status에서 뒤로가기하면 항상 Home으로 가는지
- [x] **Searching/Mission Status에 경과 시간 표시** (사용자 제안 · 2026-08-30) — 요청자가 히어로 못 구한 채 기다리는 동안, 15분(`SEARCH_TIMEOUT_MS`)이라는 마감이 있다는 걸 전혀 모른 채 "찾는 중"만 보다가 갑자기 타임아웃 결과만 받게 되던 문제.
  - **남은 시간(카운트다운)이 아니라 지난 시간으로**: "5분 남음"처럼 실패까지 남은 시간을 보여주면 이미 스트레스 상황(바퀴벌레!)에 압박감을 더하는 꼴이라, "찾는 중... N분째"처럼 진행 상황으로 프레이밍. 15분이라는 마감 숫자 자체도 노출 안 함 — `SEARCH_TIMEOUT_MS`는 "실사용 데이터 없는 추정치"라 나중에 바뀔 값이라서
  - **1분 전엔 숫자 없이, 1분부터 분 단위 표시**: "0분째"가 어색해서(아무 정보도 없는 표현) 첫 1분은 기존처럼 숫자 없는 문구, 1분부터 경과 분 표시. 처음엔 "2분 지나서부터 표시"로 설계했다가, 실제로 Searching 화면이 몇 초 만에 사라지는 걸 관찰하고 "어차피 짧으면 처음부터 보여줘도 노이즈 안 됨" 판단으로 기준 제거 — 그런데 그 "몇 초 만에 사라짐" 관찰 자체가 별개의 진짜 버그(`RequestScreen`의 포커스 가드 버그, 바로 위 항목들)였다는 게 나중에 밝혀짐. 버그 수정 후에도 이 설계(1분 기준)는 그대로 유지하기로 함
  - `src/utils/missionExpiry.ts`에 `elapsedMinutes()` 추가. `isMissionStalled()`와 같은 한계 — 렌더 시점 `Date.now()` 계산이라 실시간으로 째깍이지 않고 폴링/실시간 갱신 시점에만 업데이트됨(분 단위라 문제없다고 판단)
  - `SearchingScreen.tsx`: "Stay calm. Help is on the way." 제거하고 "Searching... / 찾는 중..." → 1분부터 "Searching... N min / 찾는 중... N분째"로 교체
  - `MissionScreen.tsx`(`requested` 상태): "We're looking for your hero."는 1분간 유지, 1분부터 "Still looking for your hero. N min so far. / 히어로를 계속 찾고 있어요. N분째예요."로 교체
  - 검증: `npx tsc --noEmit` / `npx eslint .` 통과. 실기기 확인 필요 — 1분 경계에서 문구가 실제로 바뀌는지(폴링 주기상 최대 30초 지연 가능)
...
- [x] **`requested` 미션의 서버 스케줄러 자동 만료** (사용자 제안 · 검증 완료 2026-08-27) — 기존 opportunistic 만료(15분 지난 `requested` 미션을 화면 로드 시 취소)는 **요청자 본인이 자기 화면을 다시 열어야만** 동작함. `NearbyMissionsScreen.tsx`(히어로가 보는 화면)엔 이 체크가 아예 없어서, 요청자가 요청만 하고 앱을 나가버리면 죽은 요청이 히어로 목록에 계속 살아있었음 — 위 항목에서 "서버 스케줄러 필요"라고 미뤄뒀던 게 실제로 필요한 상황이었음
  - `0017_expire_stale_requests_cron.sql`: Supabase `pg_cron` 확장으로 5분마다 `requested` + 15분(`SEARCH_TIMEOUT_MS`) 경과 미션을 자동 `cancelled` 처리. 새 인프라(Edge Function 등) 없이 Postgres 확장만으로 해결 — 범위는 `requested`만, `accepted` 이후는 위 항목의 결정대로 그대로 안 건드림
  - **막힐 뻔한 것**: `0012`의 취소 규칙 트리거가 "요청자 본인이거나 관리자만 취소 가능"이라 `auth.uid()`를 확인하는데, `pg_cron`은 JWT 없이 도니까 `auth.uid()`가 null — 스케줄러 자신이 만든 취소 요청도 트리거가 막아버림. `auth.uid() is not null and ...`로 null(=SQL Editor·서비스 접근과 동급의 신뢰 경로) 예외 추가해서 해결(`0015`의 관리자 트리거와 같은 신뢰 모델)
  - `mission_cancellations`(위 취소 패널티용 로그)엔 안 남음 — 그 트리거가 `auth.uid() = requester_id`일 때만 기록하는데 cron은 null이라 자동으로 빠짐. 타임아웃 자동 만료는 "누가 걸어나간" 신뢰 신호가 아니므로 의도한 대로.
  - 검증: 실제 REST로 만든 QA 테스트 미션이 며칠 뒤 `cancelled`로 바뀌어 있는 것을 `updated_at` 타임스탬프로 확인(자동 실행 증거)
  - **후속: 자동 만료와 요청자 직접 취소 구분** (사용자 제안 · 2026-08-27) — 위 스케줄러를 만들고 나니, `MissionScreen`이 "취소됨"만 보여주고 왜 취소됐는지가 없어서 자동 만료된 요청자가 "내가 뭘 잘못했나?"로 헷갈릴 수 있는 문제 발견. `0018_mission_cancelled_reason.sql`: `missions.cancelled_reason`(`requester`/`timeout`/`admin`, nullable) 추가 + `0017`의 cron 재등록(`cron.unschedule` 후 `cancelled_reason='timeout'` 포함해서 재스케줄 — 이미 실행 중인 job은 SQL을 값으로 들고 있어서 마이그레이션 파일만 바꿔선 안 바뀜)
...
- [x] **회원탈퇴 (soft delete)** (사용자 제안 · 2026-08-31, `notme-app` 쪽 완료) — 탈퇴 기능이 아예 없었음. 실제 DELETE 대신 `profiles.is_active`로 soft-delete하기로 결정(`missions`/`reviews` DELETE RLS 안 만든 것과 같은 이유 — 실 삭제하면 그 사람이 완료한 미션·받은 리뷰가 cascade로 같이 지워져서 다른 사람 신뢰 기록까지 날아감)
  - **의견 나눈 지점**: 자기탈퇴 후 어드민이 복구 가능하게 할지 — 실제 프로덕트(인스타/트위터 등)는 보통 "정지"만 복구 가능하고 "탈퇴"는 최종이라 구분하는데, 이 앱은 어드민이 곧 운영자 본인이고 친구들 규모라 기술적으로는 복구 가능하게 두되(단순함 우선), 화면 문구는 "복구 안 됨"으로 명확히 경고하는 절충으로 결정
  - `0019_self_deactivate_account.sql`: 기존 `0015`의 "본인 is_active 변경 전면 차단" 트리거를 방향성 있게 수정 — **끄는 것(탈퇴)은 허용, 켜는 것(자가복구)은 계속 차단**. `profiles.deactivated_reason`(`self`/`admin`) 추가, 클라이언트 입력 안 믿고 트리거가 어느 분기를 탔는지로 서버가 항상 자동 설정(위조 불가 — `mission_cancellations`의 actor 기록 방식과 같은 이유)
  - `AuthGate`(`app/_layout.tsx`)에서 전역으로 `is_active` 감시 — Supabase Auth 세션 자체는 `is_active`랑 무관하게 살아있어서(이 필드는 그냥 `profiles` 테이블 값이라 Auth가 모름), 앱이 감지해서 강제 로그아웃 + `/account-deleted` 화면으로 유도하는 구조. 어드민이 아무 때나 남을 비활성화해도(자기탈퇴가 아니어도) 잡히게 일부러 전역으로 둠
  - `SettingsScreen.tsx`: "Delete Account · 계정 탈퇴" → `BottomSheet` 확인(되돌릴 수 없다는 경고 문구) → 삭제 진행. 실패 시(오프라인 등) 성공한 것처럼 조용히 넘어가지 않고 에러 표시(미션 취소랑 다르게, 여긴 실패했는데 성공한 척하면 유저가 "삭제됐다"고 착각한 채로 계속 로그인돼 있는 이상한 상태가 됨)
  - 검증(REST로 QA 계정 사용, 실제 RLS·트리거 경로): ①자가 비활성화 → 204 성공, `deactivated_reason`이 클라 입력 없이 서버가 자동으로 `"self"`로 채워짐 확인 ②같은 계정으로 재활성화 시도 → 403 `Cannot reactivate your own account` 확인 ③비활성화 후에도 Auth 로그인 자체는 200으로 여전히 성공(세션·`is_active` 분리 의도대로) ④비활성화 상태에서 이름 등 다른 필드 수정은 정상 동작(트리거가 `is_active`/`is_admin`만 건드리는 것 확인). `npx tsc --noEmit`/`npx eslint .` 통과
  - **notme-admin 쪽은 별도 세션에서 진행 예정** — 유저 목록에 `deactivated_reason` 표시 + 비활성 계정 "복구" 액션(그냥 `is_active: true`로 업데이트하면 트리거가 `deactivated_reason`도 자동으로 null 처리). 어드민이 남을 비활성화할 때도 `deactivated_reason`을 직접 안 보내도 서버가 자동으로 `'admin'`으로 채움
  - **Settings 화면 재구성** (사용자 지적 · 2026-08-31) — 탈퇴 버튼을 급하게 알약 버튼으로 붙이면서 위쪽 설정 목록(카드+행 스타일)이랑 시각 언어가 부딪힘. DESIGN.md에 danger 색은 "파괴적 행동에만" 쓰라고 명시된 것도 안 지켜져서, Sign Out/Delete Account/dev 디버그 버튼이 전부 똑같은 무게로 보임. 실제 토큰으로 3안 목업(한 카드 통합 / 섹션 분리 / Danger Zone 격리) 만들어서 비교 후 **섹션 분리안**으로 결정 — "Preferences"/"Account" 두 섹션으로 나누고(기존 `SectionHeader` 재사용), Delete Account는 확인 시트 밖에서는 빨간 텍스트 행으로만(꽉 찬 빨간 버튼은 확인 시트 안에서만), dev 디버그 버튼은 작은 회색 텍스트 링크로 축소
  - **⚠️ 실기기 테스트 중 발견 — 탈퇴 후 `/account-deleted` 대신 그냥 로그인 화면으로 감, 스플래시 에러도 같이 발생** (2026-09-01): `useDeleteAccount`가 `is_active` 끈 직후 **직접 `signOut()`을 호출**하게 짜여 있었는데, 이게 `AuthGate`보다 먼저 세션을 null로 만들어버림. `AuthGate`는 "세션 있음 + `is_active=false`"일 때만 `/account-deleted`로 보내는데, 그 순간이 오기도 전에 세션이 이미 사라져서 그냥 "세션 없음 → 로그인 화면" 경로로 새버림 — 의도한 화면이 아예 안 뜸. 이 급격한 리다이렉트 전환(AuthGate 판단 시도 → 세션 null로 인한 재판단)이 스플래시 스크린 네이티브 에러(`No native splash screen registered...`)까지 같이 유발한 것으로 추정
    - 수정: `useDeleteAccount`에서 직접 `signOut()` 호출 제거, 대신 mutation 성공 후 `['profile', userId]` 쿼리만 무효화. `AuthGate`가 그 즉시 `useProfile()` 재조회해서 세션이 살아있는 상태에서 `is_active=false`를 정상적으로 포착 → `/account-deleted` 리다이렉트 + signOut을 AuthGate 한 곳에서만 처리하도록 단일화(설계 의도대로 되돌림)
    - 검증: `npx tsc --noEmit`/`npx eslint .` 통과. **실기기 재확인 필요** — 이번 수정으로 `/account-deleted` 화면이 실제로 뜨는지, 스플래시 에러도 같이 사라지는지
  - **실기기 확인 완료(2026-09-01~04)** — Settings 새 레이아웃, 탈퇴 → `/account-deleted` 유도 전부 정상 동작. 탈퇴 시 콘솔에 `No native splash screen registered...` 에러가 같이 뜨는데, 확인해보니 Expo 자체의 오래된 iOS 알려진 이슈([expo#15031](https://github.com/expo/expo/issues/15031), [expo#22336](https://github.com/expo/expo/issues/22336) 등) — 화면은 정상 렌더링되고 기능에 영향 없어서 그냥 두기로 함(고칠 수 있는 쪽 코드가 아님)
- [x] **히어로 승인제 — 가입만 하면 바로 미션 수락 가능했던 문제** (사용자 제안 · 2026-09-14, `notme-app` 쪽 완료) — 이 앱은 낯선 사람이 집에 들어오는 서비스인데, 정작 "누가 히어로가 될 수 있는가"는 아무 검증이 없었음(가입=요청자/히어로 둘 다 즉시 가능, 별도 히어로 역할·가입 절차 자체가 없음). 포트폴리오 서사로 "trust & safety를 고민했다"를 넣고 싶다는 동기로 진행 — 지금은 친구들끼리라 사장님이 이미 전원을 아는 사이지만, 아는 사람 범위를 넘어서기 시작하면 실제로 의미 있어지는 안전장치
  - `0022_hero_approval.sql`: `profiles.hero_approved`(default `false`) 추가, **기존 유저는 전부 `true`로 백필**(이미 쓰던 친구들 활동 안 끊기게 — 소급 차단은 안전 개선이 아니라 회귀임)
  - `0015`의 "Inactive users cannot claim missions"랑 완전히 같은 모양으로 restrictive 정책 추가해서 미승인 유저는 미션 수락(claim) 자체가 DB 레벨에서 막힘
  - **화면만 가리는 걸로는 부족하다는 걸 설계 중 발견**: Nearby Missions 목록을 안 보여줘도, 기존 SELECT 정책(`0001`)이 "열린 요청은 아무 로그인 유저나 조회 가능"이라 미승인 유저도 REST로 직접 조회하면 요청자 주소가 그대로 노출됨 — 승인제의 목적 자체가 무너지는 구멍이라, **SELECT에도 restrictive 정책 추가**(본인 요청/본인이 배정된 미션은 항상 보이되, "열린 요청 전체 브라우징"만 승인자로 한정)
  - 본인이 자기 `hero_approved`를 못 바꾸게 트리거 확장 — `is_active`(자가 탈퇴는 허용)와 다르게 **양방향 다 차단**(승인엔 합법적인 자가서비스 케이스가 없음). 어드민이 남을 승인 처리할 때 건드릴 수 있는 필드에 `hero_approved`도 추가
  - `NearbyMissionsScreen.tsx`: `hero_approved`가 false면 목록 대신 "Pending hero approval · 승인 대기 중" 화면 표시(목록 자체를 안 그림 — 이미 DB에서도 막혀 있지만 클라에서도 이중으로 안 보여줌)
  - **의도적으로 스코프 밖으로 둔 것**: 미션 상세 화면(`/hero/[id]`)에 딥링크로 직접 들어가는 경로는 안 막음 — 정상 네비게이션으론 도달 불가능하고, DB는 어차피 막고 있어서 실제 수락은 안 됨(에러 문구만 부정확하게 뜸)
  - 검증: `npx tsc --noEmit`/`npx eslint .` 통과. **cron과의 상호작용 검토** — pg_cron(0017)이 `postgres` 소유자 권한으로 도는데, `missions`에 `FORCE ROW LEVEL SECURITY`가 걸려있지 않아 RLS 정책 자체를 안 거침(정책과 달리 트리거는 소유자도 항상 거치는데, 이번에 건드린 트리거는 `profiles` 쪽이라 무관) — 실제 REST 검증은 아직 안 함
  - **notme-admin 쪽은 별도 세션에서 진행 예정** — 유저 목록에 "승인 대기" 표시 + 승인 버튼(`hero_approved: true`로 업데이트, `is_active` 복구 액션과 같은 방식)
- [x] **전체 코드 리뷰에서 발견한 위험 버그 2건 수정** (`0023_mission_race_fixes.sql`, 2026-09-18) — 위 승인제 작업 직후 전체 검토 중 발견
  - **완료/취소된 미션이 되돌려질 수 있던 문제**: `useCancelAcceptedMission`(히어로 back-out)이 `hero_id`만 필터링하고 status는 안 봄 + `0001`의 원래 정책이 미션이 어떤 status든 수정 허용 → Complete 버튼과 Cancel 버튼을 거의 동시에 눌러 경합시키면 DB가 `completed`에서 `requested`/`hero_id=null`로 되돌아갈 수 있었음(클라는 그것도 모르고 "You earned $X!" 표시). `protect_finished_missions` 트리거로 일반 불변식 추가 — `completed`/`cancelled` 미션은 그 이후 누구도 status를 못 바꿈(OLD/NEW 비교라 트리거 필수, RLS `WITH CHECK`로는 불가)
  - **`hero_approved` 회수가 진행 중인 미션까지 막던 문제**: `0022`의 restrictive claim 정책이 `hero_id is distinct from auth.uid()`로 체크했는데, 이미 배정된 히어로가 자기 미션을 진행(on_the_way/arrived/completed)할 때도 매번 재평가됨 — 나중에 관리자가 그 히어로의 `hero_approved`를 false로 바꾸면 이미 진행 중이던 미션까지 막혀버림. `enforce_hero_approval_on_claim` 트리거로 교체 — `hero_id`가 실제로 "나로" 바뀌는 순간(=claim)에만 체크, 이미 배정된 채로 상태만 바뀌는 업데이트는 체크 안 함
  - 클라 쪽도 같이 보강: `useUpdateMissionStatus`/`useCancelAcceptedMission`에 `{ count: 'exact' }` 추가해서 0건 매칭(트리거 거부 포함)을 에러로 던지게 함(기존엔 조용히 "성공" 취급), `ActiveMissionScreen`은 두 mutation 중 하나라도 진행 중이면 진행 버튼 전부 비활성화 + `handleComplete`가 실패 시 리워드 화면으로 안 넘어가게 수정
  - 검증: `npx tsc --noEmit`/`npx eslint .`/`npx prettier --check` 통과. REST로 실검증(QA 계정 + 관리자 계정) — ①completed 미션에 back-out 요청 → `23514 "A completed mission cannot be modified further"`로 거부, DB도 `completed` 그대로 확인 ②이미 미션 수락한 히어로의 `hero_approved`를 관리자가 false로 바꿔도 on_the_way→arrived→completed까지 정상 진행되지만, 같은 히어로가 **새** 미션을 수락하려 하면 0건 매칭으로 실패하고 그 미션은 `requested`/`hero_id: null` 그대로 남는 것 확인
  - PR #83, main에 머지 완료
- [x] **전체 코드 리뷰 저위험 항목 4건 마저 수정** (`0024_validate_cancelled_reason.sql`, 2026-09-19) — 위 위험 버그 수정 후 남겨뒀던 나머지 리뷰 발견 사항
  - `useSubmitReview`에 캐시 무효화 누락 — 다른 mutation 훅들과 다르게 성공 후 아무 쿼리도 무효화 안 해서, 리뷰 남긴 직후에도 히어로 평점/리뷰 개수가 새로고침 전까진 캐시된 옛 값으로 보임. `mission`/`missionHistory`/`reviews`(received)/`profile`(히어로) 무효화 추가
  - `MissionDetailScreen`의 수락 실패 메시지가 원인 불문 전부 "다른 히어로가 이미 수락했어요"로 뭉뚱그려짐 — `0023`의 `enforce_hero_approval_on_claim` 트리거가 미승인 히어로를 막을 때 `42501`(insufficient_privilege)을 던지는 걸 활용해, 이 코드일 때만 "히어로 승인 대기 중이에요"로 분리 표시. (계정 비활성화로 막히는 경우는 여전히 RLS restrictive 정책의 조용한 0건 매칭이라 원인 구분 불가 — DB 응답만으론 "이미 다른 히어로가 가져감"과 구별 불가능해서 그대로 둠)
  - `cancelled_reason`을 클라이언트가 그대로 신뢰하던 문제 — `enforce_mission_cancel_rules` 트리거에 검증 추가: 요청자 본인 취소는 `cancelled_reason`을 `'admin'`으로 못 씀, 관리자 취소는 `'requester'`/`'timeout'`으로 못 씀. (`mission_cancellations`(0013) 신뢰 신호는 `auth.uid()` 기준이라 이 문제와 무관 — 순수하게 `MissionScreen`에 뜨는 취소 사유 문구의 정직성 문제)
  - `NearbyMissionsScreen.tsx`의 "hero_approved 미승인이면 쿼리 자체를 안 쏜다"는 주석이 실제로는 거짓이었음(`useNearbyMissions()`가 무조건 실행됨) — 주석만 고치는 대신 `useNearbyMissions`에 `enabled` 파라미터를 실제로 추가해서 주석이 사실이 되게 함
  - 검증: `npx tsc --noEmit`/`npx eslint .`/`npx prettier --check` 통과. REST로 실검증(QA 계정 + 관리자 계정) — ①본인 취소를 `cancelled_reason: 'admin'`으로 위장 시도 → `42501`로 거부 ②정상 자가 취소(`'requester'`) → 정상 성공 ③관리자 취소를 `'admin'`으로 정확히 태깅 → 정상 성공 ④관리자 취소인데 `'requester'`로 잘못 태깅 → `42501`로 거부, 4가지 전부 기대대로 동작 확인
  - PR #85, main에 머지 완료
