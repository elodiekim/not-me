...
- [x] **Mission Status 아바타가 히어로가 아니라 카테고리(벌레) 아이콘으로 표시됨** (수정 완료 · 2026-07-18) — `MissionScreen.tsx`(`/mission-status`)가 `MissionCard`의 `avatar` prop에 `category.icon`(바퀴벌레 아이콘)을 넘기던 문제, "Minjun is on the way"처럼 히어로 이름이 뜨는 카드인데 아바타는 벌레라 텍스트-이미지가 안 맞던 어색함 해결.
  - `useMission.ts`: `hero:profiles!missions_hero_id_fkey(...)` select에 `avatar_url` 추가, `MissionWithRequester`에 `heroAvatarUrl: string | null` 필드로 매핑
  - `MissionScreen.tsx`: `avatar={category.icon}` → `mission.heroAvatarUrl ? { uri: mission.heroAvatarUrl } : require('../../../assets/icons/profile.png')`(제네릭 프로필 아이콘, 브랜드 옐로우) — `requested`(히어로 미배정)일 때도 카테고리 아이콘으로 안 돌아가고 이 로직 그대로 적용되게 분기 없이 처리. Mission 탭/Nearby Missions 등 "미션 종류"를 보여줘야 하는 다른 화면은 손대지 않음(그 화면들은 category 아이콘이 맞는 용도)
  - 검증(임시 테스트 계정 2개, expo web + Playwright, 실 REST): ① `requested`(히어로 없음) 진입 시 벌레 아이콘 아님 + 제네릭 프로필 아이콘 렌더 확인 ② `accepted`(히어로 배정, `avatar_url` 아직 null) 상태에서도 벌레 아이콘 아님 + 제네릭 아이콘 유지 확인 ③ (선택 항목도 진행) 테스트 계정 `profiles.avatar_url`을 REST로 실제 이미지 URL로 채운 뒤 그 URL이 그대로 `<img src>`로 렌더되는 것까지 스크린샷으로 확인 ④ 회귀: Mission 탭(`cockroach.png`)·Nearby Missions(히어로 계정, 카드 다수)에서 카테고리 아이콘이 그대로 나오는지 `<img>` src 전수 확인. `npx tsc --noEmit` 통과, 9개 체크 전부 통과
...
- [ ] **Inbox 탭에 상단 헤더가 없음** (디자인 리뷰에서 발견 · 2026-07-14) — Home(로고+벨) / Mission("My Missions") / Profile(아바타+이름)은 화면 상단에 타이틀이 있는데, Inbox는 빈 상태 아이콘+텍스트만 화면 중앙에 떠 있고 헤더가 없어서 다른 탭과 나란히 보면 미완성처럼 보임. 바로 위 "Inbox 탭: Coming Soon → 활동 피드로 교체" 작업과 함께(또는 그 전에라도) 다른 탭과 통일된 상단 헤더("Inbox / 받은편지함") 추가 필요
