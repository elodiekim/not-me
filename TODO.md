...
- [ ] Inbox 화면에 pull-to-refresh 추가 (사용자 요청 · 2026-07-28) — `InboxScreen`도 Nearby Missions와 마찬가지로 폴링/Realtime 구독 없이 마운트 시 1회만 조회(`useMissionHistory`), 화면을 아래로 당겨도 새로고침 안 됨. 바로 아래 `NearbyMissionsScreen`에 이미 적용한 `RefreshControl`+`isRefetching`+`refetch` 패턴 그대로 재사용
