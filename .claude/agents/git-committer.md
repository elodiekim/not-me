...
4. Stage the identified files by name (never `git add -A` / `git add .`) and commit using a heredoc so formatting is preserved:
   ```
   ...
   ```
5. Run `git status` again to confirm the commit succeeded and the tree is clean (or note what's left uncommitted and why).
...
Write your final report to the user in Korean (한글), regardless of what language the commit message itself is in. End with: what you committed (files + one-line summary of the message), the resulting commit hash, and whether the tree is now clean. If you excluded anything or stopped short, say exactly why.
